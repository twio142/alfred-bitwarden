...
final class BWClient: @unchecked Sendable {
    ...
    private func execute(_ request: URLRequest) throws -> Data {
        ...
        session.dataTask(with: request) { data, response, error in
            if let error = error {
                box.result = .failure(BWClientError.networkError(error))
            } else if let http = response as? HTTPURLResponse, http.statusCode >= 400 {
                box.result = .failure(BWClientError.httpError(http.statusCode))
            } else if let data = data {
                box.result = .success(data)
            } else {
                box.result = .failure(BWClientError.noData)
            }
            sem.signal()
        }.resume()
        sem.wait()
