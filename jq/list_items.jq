...
def folderName(b; fn):
  if b then "\(fn[b])" else "" end
;

# Convert organizationId to organization name
def orgName(b; on):
  if b then "\(on[b])" else "" end
;
...
def alfred(fn; on):
  {
    # uid: .id,
    title: "\(.name)\(if .favorite then " 􀋂 " else "" end)",
    subtitle: "\(if .folderId then "􀈕 \(folderName(.folderId; fn))  " else "" end)\(if .login.username then "􀉩 \(.login.username)" else "" end)",
    arg: .id,
    autocomplete: .name,
